...
- 🧭 [Additional Related Papers](#additional-related-papers)
...
An important subdivision in this category is between `pairwise ordering`, `signed stepwise dynamics`, `temporal-distance progress`, `advantage-style relative supervision`, and `analytical-reward distillation`. `TimeRewarder` is the clean temporal-distance reference point: it models how far apart two frames are in task progress and turns that signal into dense reward. `VLAC` is the clean pairwise reference point: it asks whether one state or transition is better than another under task conditioning. `Robo-Dopamine` goes further by explicitly modeling progress and regress magnitudes, then attempting to fuse those pairwise judgments back into a more globally usable signal. `ARM` reframes the problem again by asking whether short temporal windows reflect forward, backward, or stagnant task advantage. `Rewarding DINO` adds a fifth pattern: it starts from analytical dense rewards in simulation, freezes strong pretrained encoders, and learns a compact visual reward predictor through pairwise ranking. That makes it a useful bridge between classical analytic rewards and learned image-based reward models.
...
- a temporal-distance proxy for progress,
...
| Method | Title | Venue | Date | Code/Project | High-level labels | Key feature/finding |
| --- | --- | --- | --- | --- | --- | --- |
| [TimeRewarder](https://arxiv.org/abs/2509.26627) | TimeRewarder: Learning Dense Reward from Passive Videos via Frame-wise Temporal Distance | Arxiv | 9/2025 | [Project](https://timerewarder.github.io/) | `Core · Relative Progress · Temporal Distance · Shaping` | Frame-wise temporal distance from passive videos becomes step-wise proxy rewards for robot RL. |
...
| [SPA-RL](https://arxiv.org/abs/2505.20732) | SPA-RL: Reinforcing LLM Agents via Stepwise Progress Attribution | Arxiv | 5/2025 | [Code](https://github.com/WangHanLinHenry/SPA-RL-Agent) | `Core · Relative Progress · Stepwise Attribution · RL Shaping` | Decomposes delayed final rewards into per-step progress contributions and uses them as dense intermediate rewards for LLM-agent RL. |

### Expanded Notes

- `TimeRewarder`: Learns frame-wise temporal distances from passive videos and turns them into step-wise proxy rewards. Important because it offers a clean temporal-distance formulation of relative progress that can use both robot and human videos.
...
- `SPA-RL`: Introduces stepwise progress attribution for LLM-agent RL. Instead of learning a global completion score directly, it trains a progress estimator whose per-step contributions accumulate to match final task completion, then uses those estimated contributions together with grounding signals as dense intermediate rewards during policy optimization.
...
There are two important sub-directions inside this category. One is the `hybrid dense reward` route, represented by papers such as `RL-VLM-F`, `Real-World Offline RL-VLM-F`, `VARP`, `Adapt2Reward`, `TeViR`, `RoboReward`, `Robometer`, `RoboTracer`, `Large Reward Models`, `Generalizable Dense Reward for Long-Horizon Robotic Tasks`, `ReWorld`, and `LifeLong-RFT`, where VLM preferences, generated futures, process labels, and failure signals are combined to absorb richer task structure. The other is the `boundary / verifier` route, represented by `VLA-RFT`, `WorldEval`, `PRM-as-a-Judge`, and to a lesser extent `World4RL`, where the key move is to stop forcing one scalar to do everything and instead separate dense guidance from correctness checking.
...
- whether reward comes from VLM preference labels, generated future trajectories, or mixed process/outcome supervision,
...
| Method | Title | Venue | Date | Code/Project | High-level labels | Key feature/finding |
| --- | --- | --- | --- | --- | --- | --- |
| [RL-VLM-F](https://arxiv.org/abs/2402.03681) | RL-VLM-F: Reinforcement Learning from Vision Language Foundation Model Feedback | ICML 2024 | 2/2024 | [Project](https://rlvlmf2024.github.io/) | `Hybrid · VLM Preference Reward · Pairwise Preferences · Shaping + RL` | VLM preference queries are converted into learned reward functions for robot RL without human labels. |
...
| [Offline RL-VLM-F](https://arxiv.org/abs/2411.05273) | Real-World Offline Reinforcement Learning from Vision Language Model Feedback | IROS 2025 | 11/2024 | [Project](https://offline-rlvlmf.github.io/) | `Hybrid · VLM Preference Reward · Offline Reward Labeling · Offline RL` | VLM preference feedback labels unlabeled offline robot datasets with rewards, enabling downstream offline RL. |
| [VARP](https://arxiv.org/abs/2503.13817) | VARP: Reinforcement Learning from Vision-Language Model Feedback with Agent Regularized Preferences | Arxiv | 3/2025 | - | `Hybrid · VLM Preference Reward · Trajectory Preferences · Shaping + RL` | Trajectory sketches and agent-regularized reward learning improve VLM preference reliability and downstream robot RL. |
| [TeViR](https://arxiv.org/abs/2505.19769) | TeViR: Text-to-Video Reward with Diffusion Models for Efficient Reinforcement Learning | Arxiv | 5/2025 | - | `Hybrid · Generative Dense Reward · Distance + Progress + Exploration · Shaping + RL` | Text-to-video diffusion generates imagined expert trajectories that are converted into dense rewards for manipulation RL. |
...
| [PRIMO R1](https://arxiv.org/abs/2603.15600) | From Passive Observer to Active Critic: Reinforcement Learning Elicits Process Reasoning for Robotic Manipulation | Arxiv | 3/2026 | [Project](https://huggingface.co/collections/LeonOverload/primo-r1) | `Hybrid · Reasoning Rewarder · Progress + Failure · CoT Monitoring` | RL induces CoT to transform video MLLMs into active critics; initial–current boundary anchoring; strong progress estimation and zero-shot failure detection performance. |
...
- `RL-VLM-F`: Important early VLM preference reward paper. It asks a VLM for pairwise observation preferences under a text goal, then learns a reward model from those labels and uses it for policy learning without human supervision.
...
- `Real-World Offline RL-VLM-F`: Extends the same idea to unlabeled offline datasets. It matters because it turns VLM preference feedback into reward labels for suboptimal real-world robot data, then uses offline RL to learn policies without hand-labeled rewards.
- `VARP`: Improves VLM-preference reward learning by overlaying trajectory sketches and regularizing the reward model against current agent performance. Important because it makes richer trajectory evidence and policy-aware reward learning part of the supervision loop.
- `TeViR`: Uses a text-to-video diffusion model to generate an imagined expert trajectory and converts distance-to-video, progress, and exploration signals into dense rewards. Important because it adds a generative route to dense robotic reward design rather than relying only on preferences or failure prompts.
...
This section also has a useful subdivision. `IKER`, `TT-VLA`, `EVOLVE-VLA`, and `ProgressVLA` are about turning reward-like signals into task specification, online adaptation, or generation guidance, usually through shaping, differencing, or classifier-style steering. `RoboMonkey`, `RoVer`, `CoVer-VLA`, and `SV-VLA` are about test-time verification, action selection, and chunk-level replanning. `EVA` and `World Action Verifier` extend the same interface question to imagined futures and world-model rollouts, where the key issue is whether generated futures are executable and action-reachable rather than merely visually plausible. `Rewind-IL` and `SPR` treat progress or consistency anomalies as recovery and anomaly-control variables. `AdaPower` treats it as a planner-side ranking signal. These are all "reward uses," but they impose very different requirements on calibration, smoothness, and trustworthiness.
...
| Method | Title | Venue | Date | Code/Project | High-level labels | Key feature/finding |
| --- | --- | --- | --- | --- | --- | --- |
| [IKER](https://arxiv.org/abs/2502.08643) | A Real-to-Sim-to-Real Approach to Robotic Manipulation with VLM-Generated Iterative Keypoint Rewards | Arxiv | 2/2025 | - | `Interface · Reward as Task Spec · VLM-Generated Keypoints · Programmatic Shaping` | VLM-generated keypoint reward programs act as iterative task specifications for multi-step manipulation training in a real-to-sim-to-real loop. |
...
| [RoboMonkey](https://arxiv.org/abs/2506.17811) | RoboMonkey: Scaling Test-Time Sampling and Verification for Vision-Language-Action Models | CoRL 2025 | 6/2025 | - | `Interface · Runtime Verifier · Sampling + Verification · Test-Time Scaling` | VLM-based action verifier selects among sampled VLA actions to improve robustness and generalization at test time. |
...
| [Rewind-IL](https://arxiv.org/abs/2604.16683) | Rewind-IL: Online Failure Detection and State Respawning for Imitation Learning | Arxiv | 4/2026 | [Project](https://sjay05.github.io/rewind-il) | `Interface · Failure Detector + Rewind · TIDE + Safe Checkpoints · Recovery` | Training-free safeguard rewinds long-horizon imitation policies to semantically verified safe states after online failure detection. |
...
- `IKER`: Turns VLM-generated keypoint reward programs into iterative task specifications inside a real-to-sim-to-real loop. It matters because the reward is not merely learned post hoc but actively serves as the interface between language intent, simulation training, and real-world deployment.
...
- `RoboMonkey`: Uses test-time sampling plus a VLM-based action verifier to choose among candidate actions from a base VLA. Important because it makes verifier scaling, rather than policy retraining alone, a practical way to improve robustness at deployment.
...
- `Rewind-IL`: Adds a training-free safeguard to action-chunked imitation policies through zero-shot failure detection and semantically grounded respawning. It is useful because it turns recovery checkpoints into a concrete runtime control interface for rewind and retry.
...
<a id="additional-related-papers"></a>
## **🧭 Additional Related Papers**

This short section tracks additional papers that are useful context for readers of this repository, especially around value-guided planning and earlier reward-learning precursors. They are kept separate so that the main taxonomy above stays focused on the most direct robotic reward-model categories.

| Work | Why it stays outside the main tables | Why it is still useful |
| --- | --- | --- |
| [World-Value-Action Model](https://arxiv.org/abs/2604.14732) (`Arxiv`, `4/2026`) | Centers on world-model planning with a learned trajectory value function rather than on learning a robotic reward model. | Useful boundary paper for `value as system interface` and long-horizon VLA planning. |
| [CLIP-Motion](https://arxiv.org/abs/2311.03485) (`Arxiv`, `11/2023`) | Early and short-horizon consecutive-observation reward learning, with less emphasis on long-horizon manipulation semantics or failure-aware reward systems. | Useful historical precursor for motion-conditioned reward learning from observation pairs. |
